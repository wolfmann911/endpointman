    ...
    */
    function download_file_with_progress_bar($url_file, $destination_file, &$error = array()) {
        ...
        if (preg_match('/200/', $headers[0])) {
			...
			echo "<div class='progress-bar progress-bar-striped active' role='progressbar' aria-valuenow='0' aria-valuemin='0' aria-valuemax='100' style='width:0%'>0% ("._("Complete").")</div>";
			echo "</div>";
			usleep('300');
            ...
            ?>
			<script type="text/javascript">
				$('#DivProgressBar_<?php echo $randnumid; ?> .progress-bar').css('width', '100%').attr('aria-valuenow', '100').removeClass("active").text("100% (<?php echo _("Success") ?>)");
			</script>
			<?php 
            return true;
        } else {
        	...
        	echo "<div class='progress-bar progress-bar-danger progress-bar-striped' role='progressbar' aria-valuenow='100' aria-valuemin='0' aria-valuemax='100' style='width:100%'>0% ("._("Error!").")</div>";
        	echo "</div>";
